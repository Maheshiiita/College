...
export async function studentParentAutoEntry(eventSnapshot: any, context: any) {
    ...
    if (connections != null) {
        ...
        connectionsArray.map(async (value, index) => {
            ...
            var ref1 = db.collection('Colleges').doc(country).collection(schoolCode).doc('Parents').collection(standard + division).doc(value);
            batch.set(ref1, connectionMap, { merge: true });
        });
    }

    var ref2 = db.collection('Colleges').doc(country).collection(schoolCode).doc('Students').collection(standard + division).doc(studentId);
...
export async function teacherAutoEntry(eventSnapshot: any, context: any) {
    ...
    if (isTeacher) {
        ...
        return await db.collection('Colleges').doc(country).collection(schoolCode).doc('Faculty').collection(standard + division).doc(teacherId).set(map, { merge: true });
    } else {
        return;
    }
}
